...
/*si ponemos autoincremental y luego primary key dará error*/
alter table productos add primary key(nombre);
alter table productos modify id int unsigned auto_increment not null;

alter table productos drop primary key; -- hay que borrar el autoincremento primero
alter table productos modify id int unsigned; -- borrando el autoincremento

describe productos;

#Agregar Indices ADD-DROP-INDEX
alter table productos add index i_preciocantida (precio,cantidad);
