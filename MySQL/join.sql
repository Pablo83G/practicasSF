...
#Inner Join: muestra los datos que coinciden
select p.nombre, p.descripcion, p.precio, pro.nombre from proveedor as pro
inner join productos as p
on pro.id=p.proveedorid;

#Straight join: la tabla de izquierda a derecha
select p.nombre, p.descripcion, p.precio, pro.nombre from proveedor as pro
straight_join productos as p
on pro.id=p.proveedorid;

#Funciones de agrupamiento group by
select pro.nombre, count(p.proveedorid) as 'Cantidad de Productos' -- nombre que le ponemos a esta operación
from proveedor as pro
join productos as p
on pro.id=p.proveedorid -- la tabla que los une
group by pro.nombre;
...
#Trabajando con mas de dos tablas
 ...
 #variables de usuario: permiten almacenar un valor y utilizarlo después.
 #
# @nombrevariable:= 
# int nombrevarible
 #$variable
...
# quick: chequeo rápido de una tabla
# fast:
# changed:
# medium:
# extented:
...
repair table productos; -- para reparar la tabla
